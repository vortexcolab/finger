...
fetch('https://abs.incolumitas.com/get') // fetch http headers dynamically
  ...
  .then(function (data) {
    ...
    new_results['inconsistentUserAgent'] = 'OK';
    ...
    new_results['inconsistentLanguages'] = 'OK';
...
if (err.stack.toString().includes('puppeteer')) {
  ...
  new_results['puppeteerEvaluationScript'] = 'OK';
}

new_results['webdriverPresent'] = navigator.webdriver ? 'FAIL' : 'OK';
...
if (connectionRtt === undefined) {
  ...
  new_results['connectionRTT'] = connectionRtt === 0 ? 'FAIL' : 'OK';
}
...
if ((browserName === 'Chrome' || browserName === 'Chrome Headless' || browserName === 'Headless')
  ...
  permissions().then((res) => {
    if (prototypeValues[0] !== 'hardwareConcurrency~~~function get hardwareConcurrency() { [native code] }'
      ...
      new_results['puppeteerExtraStealthUsed'] = 'OK';
    }
  }).catch((err) => {
...
// get behavioral analysis from server
// sorry boys, that logic is ultra super secret (just kidding, it's simple heuristics)
// ...but to be honest, everything in life is simple heuristics
function getBehavioralClassification(delay) {
  fetch(`https://abs.incolumitas.com/classify?key=public123`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
      frames: window.bd_client.frames,
      deviceData: window.bd_client.deviceData,
      framesCapped: window.bd_client.framesCapped,
    }),
  })
    .then(response => response.json())
    .then(data => {
      var current = JSON.parse(document.getElementById('new-tests').innerText);
      current['behavioralClassificationScore'] = data;
      document.getElementById('new-tests').innerHTML = JSON.stringify(current, null, 2);
      var resEl = document.getElementById('behavioralScore');
      resEl.innerText = data;
      var color = getColor(data);
      resEl.style.color = color;
      resEl.style.textDecoration = 'underline';
      var el = document.getElementById('behavioralSamples');
      el.innerHTML += `<p>Behavioral Bot Score after ${delay}ms: <span id="behavioralScore" style="color: ${color}">${data}</span></p>`;
    })
    .catch((error) => {
...
for (let delay of [1500, 4000, 7000, 10000, 15000]) {
  setTimeout(function () {
    getBehavioralClassification(delay);
  }, delay);
}
